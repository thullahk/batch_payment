...
class TestAccountBankStatementImportCSV(AccountTestInvoicingCommon):
    ...
    def test_csv_file_import(self):
        self._import_file('at_accounting/test_csv_file/test_csv.csv')
    ...
    def test_csv_file_import_with_missing_values(self):
        self._import_file('at_accounting/test_csv_file/test_csv_missing_values.csv', ['transaction_type', 'ref', 'payment_ref', 'debit', 'credit'])
    ...
    def test_csv_file_import_non_ordered(self):
        with self.assertRaises(UserError):
            self._import_file('at_accounting/test_csv_file/test_csv_non_sorted.csv')

    def test_csv_file_empty_date(self):
        with self.assertRaises(UserError):
            self._import_file('at_accounting/test_csv_file/test_csv_empty_date.csv')
    ...
    def test_csv_file_import_without_amount(self):
        ...
        with self.assertRaisesRegex(ValidationError, "Make sure that an Amount or Debit and Credit is in the file."):
            self._import_file('at_accounting/test_csv_file/test_csv_without_amount.csv', csv_fields)
