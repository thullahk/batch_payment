...
class ECSalesReportCustomHandler(models.AbstractModel):
    ...
    def _get_custom_display_config(self):
        return {
            'components': {
                'AccountReportFilters': 'at_accounting.SalesReportFilters',
            },
        }
            ...
            if not company_currency.is_zero(row['balance']):
                ...
                if warnings is not None:
                    if row['country_code'] not in self._get_ec_country_codes(options):
                        warnings['at_accounting.sales_report_warning_non_ec_country'] = {'alert_type': 'warning'}
                    elif not row.get('vat_number'):
                        warnings['at_accounting.sales_report_warning_missing_vat'] = {'alert_type': 'warning'}
                    if row.get('same_country') and row['country_code']:
                        warnings['at_accounting.sales_report_warning_same_country'] = {'alert_type': 'warning'}
