...
class AccountReport(models.Model):
    ...
    def caret_option_open_general_ledger(self, options, params):
        ...
        general_ledger = self.env.ref('at_accounting.general_ledger_report')
        lines_to_unfold = []
        ...
        action_vals = self.env['ir.actions.actions']._for_xml_id('at_accounting.action_account_report_general_ledger')
        action_vals['params'] = {
            'options': gl_options,
            'ignore_session': True,
        }
    ...
    def _generate_common_warnings(self, options, warnings):
        ...
        if options.get('available_tax_units') and options['tax_unit'] == 'company_only':
            warnings['at_accounting.common_warning_tax_unit'] = {}
        ...
        if self.filter_multi_company == 'tax_units' and any(accessible_branch.id not in report_company_ids for accessible_branch in self.env.company._accessible_branches()):
            warnings['at_accounting.tax_report_warning_tax_id_selected_companies'] = {'alert_type': 'warning'}
        ...
        if options.get('date') and options.get('all_entries') is not None:
            if self.env['account.move'].search_count(
                ...
                warnings['at_accounting.common_warning_draft_in_period'] = {}
    ...
    def action_display_inactive_sections(self, options):
        ...
        return {
            ...
            'views': [(False, 'list'), (False, 'form')],
            'context': {
                'list_view_ref': 'at_accounting.account_report_add_sections_tree',
                'active_test': False,
            },
        }
    ...
    def export_to_pdf(self, options):
        ...
        footer = self.env['ir.actions.report']._render_template("at_accounting.internal_layout", values=rcontext)
    ...
    def _get_pdf_export_html(self, options, lines, additional_context=None, template=None):
        ...
        template = custom_print_templates.get('pdf_export_main', 'at_accounting.pdf_export_main')
        ...
        for report in self:
            report.is_account_coverage_report_available = (
                ...
                self.root_report_id in (
                    self.env.ref('at_accounting.profit_and_loss', raise_if_not_found=False),
                    self.env.ref('at_accounting.balance_sheet', raise_if_not_found=False)
                )
            )
    ...
    def _generate_accounts_coverage_report_xlsx_lines(self):
        ...
        if self.root_report_id == self.env.ref('at_accounting.profit_and_loss'):
